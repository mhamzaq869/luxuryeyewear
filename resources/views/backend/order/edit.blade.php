    ...
    <form action="{{route('order.update',$order->id)}}" method="POST">
      ...
      <div class="form-group">
        ...
        <select name="status" id="" class="form-control">
          ...
          <option value="process" {{($order->status=='delivered'|| $order->status=="cancel") ? 'disabled' : ''}}  {{(($order->status=='processing')? 'selected' : '')}}>Processing</option>
          <option value="dispatch" {{($order->status=="cancel") ? 'disabled' : ''}}  {{(($order->status=='delivered')? 'selected' : '')}}>Dispatch</option>
