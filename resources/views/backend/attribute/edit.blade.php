    ...
    <div class="p-3 ">
        ...
        <form method="post" action="{{ route('attribute.update', $attribute->id) }}">
            ...
            <div class="form-group">
                ...
                <input id="inputTitle" type="text" name="name" value="{{ $attribute->name }}"
                    placeholder="Enter Name" value="{{ old('name') }}" class="form-control">
                @error('name')
            ...
            <div class="form-group">
                ...
                <select name="attribute_type" id="attribute_type" class="form-control">
                    <option value="product_for" {{ $attribute->attribute_type == 'product_for' ? 'selected' : '' }}>
                        product_for</option>
                    <option value="shape" {{ $attribute->attribute_type == 'shape' ? 'selected' : '' }}>shape</option>
                    <option value="material" {{ $attribute->attribute_type == 'material' ? 'selected' : '' }}>material
                    </option>
                    <option value="type" {{ $attribute->attribute_type == 'type' ? 'selected' : '' }}>type</option>
                    <option value="lens_type" {{ $attribute->attribute_type == 'lens_type' ? 'selected' : '' }}>
                        lens_type</option>
                    <option value="extra" {{ $attribute->attribute_type == 'extra' ? 'selected' : '' }}>extra</option>
                    <option value="order_status" {{ $attribute->attribute_type == 'order_status' ? 'selected' : '' }}>
                        Order Status</option>
                </select>
            </div>
