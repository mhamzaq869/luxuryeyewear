        ...
        <div class="col-md-6 col-xl-4">
          ...
          <div class="cardStyle1">
                          ...
                          @if (isset($i) && $i >= 2)
                              ...
                              <li>
                                  <a href="{{route('product-detail',[$product->slug])}}" target="_blank" class="text-danger m-2">
                                    @if (count($product_variant->where('cat_id',$product->cat_id)->whereIn('product_for', [$product->product_for,30])) - 4 > 0)
                                        +{{count($product_variant->where('cat_id',$product->cat_id)->whereIn('product_for', [$product->product_for,30])) - 4}}
                                    @endif
                                  </a>
                              </li>
                              @endif
              ...
              <div class="contentCol">
                  ...
                  <h4 class="brandCol" id="female_eyeglass_brand_name_{{ $product->id }}">{{ $product->brand->title }} </h4>
                  ...
                  <span class="priceCol" id="female_eyeglass_pro_price_{{ $product->id }}"> ${{ ceil($product->price) }}</span>
